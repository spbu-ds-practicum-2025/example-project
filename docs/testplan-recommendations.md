...
- Предусловия для всех тестов (при их наличии)
...
**Предусловия**:
- В каком состоянии должна находиться система для успешного прохождения теста
...
**Ожидаемый результат**:
- Ожидаемое возвращаемое значение тестируемой функции
- Ожидаемое состояние, в которое система должна перейти после выполнения действия
```
...
### 3.3. Проверка побочных эффектов операции

**Замечание:** Побочный эффект - изменение в состоянии системы, которые произошли при вызове API.
Их может не быть, если вы тестируете метод API, возвращающий актуальное состояние некоторого ресурса.
Однако часто методы API приводят к изменениям в состоянии системы: создание нового ресурса, изменение состояния ресурса и т.д.
И корректность таких побочных эффектов тоже следует проверять.
...
2. **Основные ошибки бизнес-логики** — показывает обработку ошибок
...
6. **Дополнительные ошибки бизнес-логики** — для демонстрации покрытия
