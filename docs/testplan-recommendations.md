...
6. **Конкурентность** — если есть параллельные запросы
7. **Дополнительная ошибка бизнес-логики** — для демонстрации покрытия
8. **Последовательность операций** — для демонстрации работоспособности
...
```
1. Успешные позитивные сценарии (Happy Path)
...
4. Прочие тесты: идемпотентность, конкурентность, согласованность
```
